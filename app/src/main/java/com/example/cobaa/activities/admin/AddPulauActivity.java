...
public class AddPulauActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        title.setText("Tambah Data Pulau");
        EditText name_pulau = findViewById(R.id.name_banner);
        name_pulau.setHint("Nama Pulau");
        imageView = findViewById(R.id.image);

        Button save = findViewById(R.id.save);
        save.setOnClickListener(v -> cekInputan(name_pulau));

        ImageView iv_back = findViewById(R.id.iv_back);
        iv_back.setOnClickListener(v -> finish());
    }

    private void cekInputan(EditText name_pulau) {
        if (name_pulau.getText().toString().trim().isEmpty()){
            name_pulau.setError("Nama Pulau Tidak boleh kosong");
            name_pulau.requestFocus();
            return;
        }
        if (foto_dari.equals("gallery")){
            saving(name_pulau);
        }else{
            savingFromCamera(name_pulau);
        }
    }
    ...
    private void savingFromCamera(EditText name_pulau) {
        if (resultUri != null || bitmap != null) {
            ...
            final StorageReference storageReferences = storageReference.child("icon/pulau/"+ name_pulau.getText().toString().trim());
